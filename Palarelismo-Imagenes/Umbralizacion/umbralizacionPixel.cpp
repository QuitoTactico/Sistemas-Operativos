...
vector<vector<Pixel>> umbralizarSec(vector<vector<Pixel>> matriz, int umbral){
    int filas = matriz.size();
    int columnas = matriz[0].size();

    vector<vector<Pixel>> matrizUmbralizada(filas, vector<Pixel>(columnas));
...
int main(int argc, char* argv[]){
    ...
    start = omp_get_wtime();
    vector<vector<Pixel>> matrizUmbralizadaSec = umbralizarSec(matriz, umbral); 
    end = omp_get_wtime();
